...
import {Extrinsic, EventRecord, SignedBlock} from '..'

/** Host Substrate interface */
// export declare namespace substrate {
...
export namespace substrate {
  /** Type hint for Substrate values. */
  ...
  /**
   * Pointer type for Substrate value data.
   *
  ...
  /**
   * A dynamically typed value used when accessing Substrate data.
   */
  export class Value {
    kind: ValueKind
    data: ValuePayload
    ...
    toAddress(): Address {
      assert(this.kind == ValueKind.ADDRESS, 'Substrate value is not an address')
      return changetype<Address>(this.data as u32)
    }

    toBoolean(): boolean {
      assert(this.kind == ValueKind.BOOL, 'Substrate value is not a boolean.')
      return this.data != 0
    }
    ...
    toI32(): i32 {
      assert(
        ...
        'Substrate value is not an int or uint.',
      )
    ...
    toString(): string {
      assert(this.kind == ValueKind.STRING, 'Substrate value is not a string.')
    ...
    toArray(): Array<Value> {
      assert(
        ...
        'Substrate value is not an array.',
      )
      return changetype<Array<Value>>(this.data as u32)
    }

    toTuple(): Tuple {
      assert(this.kind == ValueKind.TUPLE, 'Substrate value is not a tuple.')
    ...
    toBooleanArray(): Array<boolean> {
      assert(
        this.kind == ValueKind.ARRAY || this.kind == ValueKind.FIXED_ARRAY,
        'Substrate value is not an array or fixed array.',
      )
      let valueArray = this.toArray()
  ...
  /**
   * Common representation for Substrate tuples / Solidity structs.
   *
  ...
  /**
   * Common representation for Substrate smart contract calls.
   */
  export class Call {
    to: Address
    from: Address
  ...
  /**
   * Common representation for Substrate smart contract events.
   */
  export class Event {
    address: Address
    logIndex: BigInt
  ...
  /**
   * A dynamically-typed Substrate event parameter.
   */
  ...
  /**
   * Low-level interaction with Substrate smart contracts
   */
  export class SmartContract {
    ...
    call(name: string, signature: string, params: Array<Value>): Array<Value> {
      ...
      let result = substrate.call(call)
      assert(
        result != null,
